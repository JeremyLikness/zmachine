...
func command(this js.Value, args []js.Value) interface{} {
	if !zm.done {
		check := args[0].String()
		if len(check) > 0 {
			nextcommand = check
...
func zork(this js.Value, args []js.Value) interface{} {
	buffer := typedArrayToByteSlice(args[0])
	window := js.Global()
	var header ZHeader
	header.read(buffer)
	...
	loop = js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		if !zm.done {
			...
			window.Call("requestAnimationFrame", loop)
		}
		return nil
	})

	window.Call("setTimeout", loop)
	return nil
}
...
func main() {
	...
	zorkFunc := js.FuncOf(zork)
	commandFunc := js.FuncOf(command)
	js.Global().Set("zork", zorkFunc)
	js.Global().Set("command", commandFunc)
	defer zorkFunc.Release()
	defer commandFunc.Release()
	<-done
}
